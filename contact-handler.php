...
try {
    // SMTP configuration - Using Namecheap SMTP (cPanel recommended settings)
    $mail->isSMTP();
    $mail->Host       = 'prismaticpractice.com'; // Domain-based SMTP server (from cPanel)
    $mail->SMTPAuth   = true;
    ...
    // Email addresses - Proper From address is critical for spam filtering
    ...
    // Set proper Date header (PHPMailer does this automatically, but ensure it's set)
    $mail->MessageDate = date('r');
    
    // Add proper headers to improve deliverability and reduce spam filtering
    // Message-ID is automatically generated by PHPMailer
    $mail->addCustomHeader('X-Mailer', 'Prismatic Minds Contact Form');
    $mail->addCustomHeader('X-Priority', '3');
    // Remove headers that might trigger spam filters
    // Don't use 'Precedence: bulk' or 'Auto-Submitted' as they can trigger spam filters
